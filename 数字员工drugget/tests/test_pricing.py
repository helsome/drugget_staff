...
def entry(spec: str | None, price: str, *, confirmed: bool = True, designated: bool = False) -> ControlPriceEntry:
    return ControlPriceEntry(
        ...
        authority_basis="designated_source" if designated else None,
    )
...
def test_control_price_uses_strength_as_selector_not_divisor() -> None:
    entries = [entry("20mg", "1.65", confirmed=False, designated=True), entry("50mg", "3.30", confirmed=False, designated=True)]
    assert resolve_control_price(entries, brand="希诺彤", spec="20mg*14粒", on_date=date(2026, 7, 20)).price == Decimal("1.65")
...
def test_non_designated_unconfirmed_rules_are_ineligible() -> None:
    ...
    assert resolve_control_price(
        [entry("20mg", "1.65", confirmed=False)],
        brand="希诺彤",
        spec="20mg*7粒",
        on_date=date(2026, 7, 20),
    ) is None
...
def test_full_spec_business_confirmation_overrides_designated_strength() -> None:
    entries = [
        entry("20mg", "1.65", confirmed=False, designated=True),
        entry("20mg*14粒", "1.80", confirmed=True),
    ]
    assert resolve_control_price(entries, brand="希诺彤", spec="20mg*14粒", on_date=date(2026, 7, 20)).price == Decimal("1.80")


def test_same_priority_rules_fail_closed_as_ambiguous() -> None:
    with pytest.raises(AmbiguousControlPrice):
        resolve_control_price(
            [entry("20mg", "1.65", confirmed=False, designated=True), entry("20mg", "1.70", confirmed=False, designated=True)],
            brand="希诺彤", spec="20mg*14粒", on_date=date(2026, 7, 20),
        )


def test_existing_getai_and_tuotuo_rules_keep_confirmation_separate_from_guidance() -> None:
    ...
    assert resolve_control_price(entries, brand="托妥", spec="10mg*48片", on_date=date(2026, 7, 20)).price == Decimal("0.80")
