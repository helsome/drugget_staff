...
| 控价匹配和破价判断 | 有 | 有 | **已对两条真实详情验证“暂不可比较”；尚无已确认完整规格规则的破价闭环** |
| 责任人路由和破价事件 | 有 dry-run | 有 | **仅自动测试；真实低价案件尚未出现** |
| 钉钉真实通知 | 没有 | 没有 | **没有** |
...
> **淘宝和药师帮的最小价格采集闭环及两条详情价标准化已验证；控价只允许完整规格且业务确认的规则，真实破价、责任通知、公司系统和规模化运行仍待验收。**
...
| 价格换算与控价判断 | 已对两条在线详情价完成判断 | 已保存页面价、单盒价、最小单位价与药师帮起购盒数；葛泰与托妥均因无“完整规格 + 已业务确认”控价而为 `not_comparable`，零破价案件 |
