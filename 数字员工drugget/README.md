...
| 规格、包装、单盒价和最小单位换算 | 有 | 有 | **两条最新在线详情已保存标准化结果** |
| 控价匹配和破价判断 | 有 | 有 | **葛泰已完成一条真实详情的严格 dry-run 判断；托妥仍暂不可比较** |
| 责任人路由和破价事件 | 有 dry-run | 有 | **葛泰低价已生成一条本地案件并进入中央待分配；未发送通知** |
| 钉钉真实通知 | 没有 | 没有 | **没有** |
...
> **淘宝和药师帮的最小价格采集闭环及两条详情价标准化已验证；葛泰已在完整规格且业务确认控价下完成一次本地 dry-run，真实通知、公司系统和规模化运行仍待验收。**
...
| 小规模测试库 | 可恢复 | 缺少 `price_observations_clean.csv` 时使用 `build_test_knowledge_base.py --rebuild-source` 在临时目录重建来源 |
...
| 价格换算与控价判断 | 已对两条在线详情价完成判断 | 葛泰 `0.45g*20片` 已按业务确认的 `1.15 元/片` 判定 `below_control`，并建立一条 dry-run 案件；托妥仍为 `not_comparable` |
| 异常人工处理 | 基础版已有 | 支持验证码、登录、限流、页面变更转人工和当前任务恢复 |
| 钉钉通知 | 暂不启用 | 仅能生成本地通知预览和中央待分配队列，绝不实际发送 |
...
| 价格解析与控价判断 | `src/price_specialist/pricing.py` + `decisions.py` | 仅匹配完整规格、有效期内且已业务确认的控价；明确输出三态结论 |
| 详情结果接控价 | `src/price_specialist/decisions.py::PriceDecisionService` | 保存判断、规则来源和详情证据；缺任一条件则 `not_comparable` |
| 破价事件与预览 | `alerts.py` + `models.py` | 仅 `below_control` 生成幂等 dry-run 案件；路由生成本地预览或中央待分配队列 |
