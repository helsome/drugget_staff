...
class PriceDecisionService:
    ...
    def evaluate_observation(self, observation_id: str, *, as_of: date | None = None) -> PriceComparison:
        ...
        if matched is None:
            return self._replace(comparison, verdict=NOT_COMPARABLE, reason_code="exact_confirmed_control_rule_missing", reason_detail=f"{brand} {spec} 在 {when.isoformat()} 没有唯一、已确认的完整规格控价", comparison_unit_price=observation.single_unit_price, control_price=None, min_unit=package_min_unit, difference=None, **common)
        rule = next(row for row in rows if row.spec_key == matched.spec_key and Decimal(row.price_per_min_unit) == matched.price and row.min_unit == matched.min_unit and row.source_line == matched.source_line)
